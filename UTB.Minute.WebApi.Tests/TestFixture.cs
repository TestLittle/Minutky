...
namespace UTB.Minute.WebApi.Tests
{
    public class TestFixture : IAsyncLifetime
    {
        ...
        public async ValueTask InitializeAsync()
        {
            ...
            var response = await keycloakClient.PostAsync("/realms/utb-minute/protocol/openid-connect/token", new FormUrlEncodedContent(new Dictionary<string, string>
            {
                { "grant_type", "password" },
        ...
        public async ValueTask DisposeAsync()
        {
            if (keycloakClient is not null)
            {
                ...
                {
                    _ = await keycloakClient.PostAsync("/realms/utb-minute/protocol/openid-connect/logout",
